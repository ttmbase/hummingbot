...
from hummingbot.connector.exchange.ttmbase.ttmbase_constants import Constants
from hummingbot.connector.exchange.ttmbase.ttmbase_in_flight_order import TtmBASEInFlightOrder
from hummingbot.connector.exchange.ttmbase.ttmbase_order_book_tracker import TtmBASEOrderBookTracker
from hummingbot.connector.exchange.ttmbase.ttmbase_user_stream_tracker import TtmBASEUserStreamTracker
from hummingbot.connector.exchange.ttmbase.ttmbase_utils import (
    TtmBASEAPIError,
...
class ttmbaseExchange(ExchangeBase):
    """
    TtmBASEExchange connects with TtmBASE exchange and provides order book pricing, user account tracking and
    ...
    def __init__(self,
        ...
        """
        :param ttmbase_api_key: The API key to connect to private TtmBASE APIs.
        :param ttmbase_secret_key: The API secret.
        ...
        self._ttmbase_auth = TtmBASEAuth(ttmbase_api_key, ttmbase_secret_key, time_provider=self._time_synchronizer)
        self._set_order_book_tracker(TtmBASEOrderBookTracker(trading_pairs=trading_pairs))
        self._user_stream_tracker = TtmBASEUserStreamTracker(self._ttmbase_auth, trading_pairs)
        self._ev_loop = asyncio.get_event_loop()
        self._shared_client = None
        self._poll_notifier = asyncio.Event()
        self._last_timestamp = 0
        self._in_flight_orders = {}  # Dict[client_order_id:str, TtmBASEInFlightOrder]
    ...
    @property
    def in_flight_orders(self) -> Dict[str, TtmBASEInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        ...
        self._in_flight_orders.update({
            key: TtmBASEInFlightOrder.from_json(value)
    ...
    async def _format_trading_rules(self, symbols_info: Dict[str, Any]) -> Dict[str, TradingRule]:
        ...
        for rule in symbols_info:
            try:
                trading_pair = await TtmBASEAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(rule["symbol"])
    ...
    async def _api_request(self,
        ...
        headers: dict = {}
        if is_auth_required:
            headers: dict = self._ttmbase_auth.get_headers(method, f"{Constants.REST_URL}/{endpoint}",
                                                          params)
        # Build request coro
        ...
        if request_errors or parsed_response is None:
            if try_count < Constants.API_MAX_RETRIES:
                ...
                raise TtmBASEAPIError({"error": parsed_response, "status": http_status})
        if "error" in parsed_response:
            raise TtmBASEAPIError(parsed_response)
    ...
    async def _create_order(self,
        ...
        symbol = await TtmBASEAPIOrderBookDataSource.exchange_symbol_associated_to_pair(trading_pair)
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = TtmBASEInFlightOrder(
    ...
    async def _execute_cancel(self, trading_pair: str, order_id: str) -> str:
        ...
        :param trading_pair: The market trading pair (Unused during cancel on TtmBASE)
    ...
    async def _update_order_status(self):
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            for response, tracked_order in zip(responses, tracked_orders):
                ...
                if isinstance(response, TtmBASEAPIError):
    ...
    async def _user_stream_event_listener(self):
        ...
        TtmBASEAPIUserStreamDataSource.
        """
    ...
    async def get_open_orders(self) -> List[OpenOrder]:
        ...
        for order in result:
            ...
            if order["type"] != OrderType.LIMIT.name.lower():
                ...
                continue
            trading_pair = await TtmBASEAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
                order["symbol"])
            ret_val.append(
                OpenOrder(
    ...
    async def all_trading_pairs(self) -> List[str]:
        ...
        return await TtmBASEAPIOrderBookDataSource.fetch_trading_pairs()

    async def get_last_traded_prices(self, trading_pairs: List[str]) -> Dict[str, float]:
        # This method should be removed and instead we should implement _get_last_traded_price
        return await TtmBASEAPIOrderBookDataSource.get_last_traded_prices(trading_pairs=trading_pairs)
