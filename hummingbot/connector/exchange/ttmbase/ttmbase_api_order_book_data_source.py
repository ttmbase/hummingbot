...
from .ttmbase_constants import Constants
from .ttmbase_order_book import TtmBASEOrderBook
from .ttmbase_utils import TtmBASEAPIError, api_call_with_retries, str_date_to_ts
from .ttmbase_websocket import TtmBASEWebsocket
...
class TtmBASEAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_last_traded_prices(cls, trading_pairs: List[str]) -> Dict[str, Decimal]:
        ...
        for trading_pair in trading_pairs:
            ex_pair: str = await TtmBASEAPIOrderBookDataSource.exchange_symbol_associated_to_pair(trading_pair)
            if len(trading_pairs) > 1:
    ...
    async def exchange_symbol_associated_to_pair(trading_pair: str) -> str:
        symbol_map = await TtmBASEAPIOrderBookDataSource.trading_pair_symbol_map()
    ...
    async def fetch_trading_pairs() -> List[str]:
        symbols_map = await TtmBASEAPIOrderBookDataSource.trading_pair_symbol_map()
    ...
    async def get_order_book_data(trading_pair: str) -> Dict[str, any]:
        ...
        """
        try:
            ex_pair = await TtmBASEAPIOrderBookDataSource.exchange_symbol_associated_to_pair(trading_pair)
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        ...
        snapshot_msg: OrderBookMessage = TtmBASEOrderBook.snapshot_message_from_exchange(
            snapshot,
            snapshot_timestamp,
        ...
        while True:
            try:
                ...
                for pair in self._trading_pairs:
                    symbol = await TtmBASEAPIOrderBookDataSource.exchange_symbol_associated_to_pair(pair)
                ...
                async for response in ws.on_message():
                    ...
                    for trade in trades_data["data"]:
                        ...
                        trade_msg: OrderBookMessage = TtmBASEOrderBook.trade_message_from_exchange(
                            trade,
                            trade_timestamp,
        ...
        while True:
            try:
                ...
                async for response in ws.on_message():
                    ...
                    order_book_msg_cls = (TtmBASEOrderBook.diff_message_from_exchange
                                          if method == Constants.WS_METHODS['ORDERS_UPDATE'] else
                                          TtmBASEOrderBook.snapshot_message_from_exchange)
