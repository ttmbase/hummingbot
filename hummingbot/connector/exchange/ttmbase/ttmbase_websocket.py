...
from hummingbot.connector.exchange.ttmbase.ttmbase_utils import RequestId, TtmBASEAPIError
...
class TtmBASEWebsocket(RequestId):
    ...
    def __init__(self,
                 auth: Optional[TtmBASEAuth] = None):
        self._auth: Optional[TtmBASEAuth] = auth
    ...
    async def connect(self):
        ...
        if self._isPrivate:
            ...
            if json_msg.get("result") is not True:
                ...
                raise TtmBASEAPIError({"error": f"Failed to authenticate to websocket - {err_msg}."})
